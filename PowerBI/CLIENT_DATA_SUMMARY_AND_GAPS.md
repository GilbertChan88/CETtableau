...
> **Interim build:** `dashboard5contentmktg` and `dashboard6oohads` are now fully built out with **DUMMY /
> mock performance data** (`Mock_ContentMarketing_Performance.csv` → `ContentMktgPerf`; `Mock_OOH_Performance.csv`
> → `OOHPerf`, generated by `scripts/gen_dummy.py`) so the layouts and charts are ready. **Replace the mock CSVs
> with the real exports** (matching the columns in Parts B/C below) to make them live. Dashboard 3 has no
> interim view (no data proxy exists).
