...
- *Platform breakdown* → the Campaign Schedule axis uses the `Campaign & Platform` composite column, so every Campaign × Marketing Platform is its own visible row/bar (a bar-chart hierarchy would hide the platform behind drill-down). Campaign Name + Marketing Platform are also in the tooltip. This is the Tableau "Individual (bar per platform)" view.
- *Schedule View (Individual/Overlap) toggle* → the reliable way to switch grain is a Power BI **field parameter**, which must be created in Power BI Desktop (Modeling ▸ New parameter ▸ Fields) with two fields — `CampaignSpend[Campaign & Platform]` labelled "Individual (bar per platform)" and `CampaignSpend[Campaign Name]` labelled "Overlap (combined)" — then drop that parameter on the schedule's axis in place of `Campaign & Platform` and add its slicer. Hand-authored field parameters do not bind on the axis (they render their option labels instead), so this step is left for Desktop.
