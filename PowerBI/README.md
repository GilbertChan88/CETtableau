...
- *Schedule View* (Individual/Overlap) → a **field parameter** (`Schedule View`) with a slicer: **Individual (bar per platform)** adds `Marketing Platform` as the schedule's second axis level (one bar per Campaign × Platform, platform shown as the sub-row + tooltip); **Overlap (combined)** collapses to one bar per Campaign spanning all its platforms.
