...
### Dashboards 5 & 6 — built on DUMMY performance data (placeholder until real exports)
The client hasn't yet supplied the content‑marketing or OOH **performance** exports, so these two pages run
on **mock/dummy data** generated by `scripts/gen_dummy.py`:
- `Mock_ContentMarketing_Performance.csv` → table **`ContentMktgPerf`** (Publisher × Campaign × Month with
  Reach, Page Views, Avg Time on Page, Impressions, Clicks, Spend).
- `Mock_OOH_Performance.csv` → table **`OOHPerf`** (OOH Format × Site × Campaign × Month with Reach,
  QR Code Scans, Impressions, Spend).

Each page has a title, Campaign/Dept/Objective/Publisher‑or‑Format/Month slicers, KPI cards, three charts,
and a detail matrix:
- **Dashboard 5 (Content Marketing):** KPIs Reach / Page Views / Avg Time on Page / Media Spend; charts
  *Reach by Publisher*, *Page Views by Month*, *Avg Time on Page by Publisher*; matrix Publisher ▸ Campaign.
  Measures: `CM Reach`, `CM Page Views`, `CM Avg Time on Page (s)` (page‑view‑weighted), `CM Impressions`,
  `CM Clicks`, `CM Media Spend`, `CM Cost per View`.
- **Dashboard 6 (OOH):** KPIs Reach / QR Code Scans / Cost per Scan / Media Spend; charts *QR Scans by
  Format*, *Reach by Site*, *Cost per Scan by Format*; matrix Format ▸ Site ▸ Campaign. Measures:
  `OOH Reach`, `OOH QR Scans`, `OOH Impressions`, `OOH Media Spend`, `OOH Cost per Scan`, `OOH Scan Rate`.

**When the real exports arrive:** match the CSV column names above, drop them in the `SourceFolder`, and
the pages work unchanged (delete the `Mock_…` files / `gen_dummy.py`). The titles/matrix labels flag the data
as DUMMY. The earlier interim `CampaignSpend` channel measures were removed. See `CLIENT_DATA_SUMMARY_AND_GAPS.md`.
